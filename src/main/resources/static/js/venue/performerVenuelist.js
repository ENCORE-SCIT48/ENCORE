...
(() => {
    ...
    async function fetchVenues({ keyword = '', page = DEFAULT_PAGE, size = DEFAULT_SIZE }) {
        ...
        try {
            ...
            if (!res.ok) { console.error('[performerVenueList] fetch 실패', res.status); render([]); return; }
            const body = await res.json();
            render(body?.data?.content || []);
        } catch (e) {
    ...
    $input.addEventListener('input', () => {
        ...
        debounceTimer = setTimeout(() => fetchVenues({ keyword: $input.value, page: 0 }), 250);
    });
    ...
    $input.addEventListener('keydown', e => {
        if (e.key === 'Enter') {
            e.preventDefault();
            clearTimeout(debounceTimer);
            fetchVenues({ keyword: $input.value, page: 0 });
        }
    });
